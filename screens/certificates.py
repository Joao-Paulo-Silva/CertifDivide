...
def certificates(root, tab):
    ...
    def divide_pdfs():
        ...
        progress_bar.grid(row=12, column=0, columnspan=1, pady=(0, 20), padx=(0, 0))
        label.grid(row=12, column=1, columnspan=2, pady=(0, 20), padx=(0, 0))
        ...
        if name_mode_var.get():
            if input_pdf_path != '':
                ...
                label.update()
                names, pastas = extract_names_from_pdf(input_pdf_path, config['certificate_name'], config['certificate_registration'], pages_search, progress_bar, past_mode_var.get())
            else:
    ...
    pages_per_division_entry.grid(row=5, column=0, columnspan=1, sticky=tk.W, pady=(0, 10))
    ...
    name_mode_check.grid(row=7, column=0, columnspan=2, sticky=tk.W, pady=(0, 10))
    ...
    ttk.Button(tab, text="Dividir PDFs", command=divide_pdfs, cursor="hand2", width=30).grid(row=13, column=0, padx=5, pady=10)
    ttk.Button(tab, text="Sair", command=root.quit, width=30, cursor="hand2", bootstyle="danger").grid(row=13, column=1, padx=5, pady=10)
